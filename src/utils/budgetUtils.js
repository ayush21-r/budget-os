...
export function calculateBudgetOverview(state) {
  ...
  const availableSpending = Number(state.profile.allowance) - Number(state.profile.savings_goal);
  const current_balance = availableSpending - spent;
  ...
  return {
    ...
    spent,
    current_balance,
    remaining_budget: availableSpending - spent,
    categoryTotals,
    highestSpendingCategory,
    savingsProgress: state.profile.allowance > 0 ? Math.min((state.profile.savings_goal / state.profile.allowance) * 100, 100) : 0,
  };
}
...
export function getMonthlySpendingData(currentState, history = []) {
  const archived = history.map((month) => ({
    ...
    spent: Number(month.profile.total_spent || 0),
    saved: Number(month.profile.savings_goal || 0),
  }));
  ...
  return [
    ...archived,
    {
      ...
      saved: Number(currentState.profile.savings_goal || 0),
    },
  ].slice(-6);
}
